...
func TestKernelComponentsInitialization(t *testing.T) {
	...
	t.Run("TripleHelixScheduler", func(t *testing.T) {
		...
		tripleHelixScheduler := scheduler.NewTripleHelixScheduler(config)

		if err := tripleHelixScheduler.Initialize(ctx); err != nil {
			t.Errorf("Failed to initialize scheduler: %v", err)
		}

		if err := tripleHelixScheduler.HealthCheck(ctx); err != nil {
		...
		if err := tripleHelixScheduler.ScheduleTask(task); err != nil {
...
func TestArbitrationEngine(t *testing.T) {
	...
	tripleHelixScheduler := scheduler.NewTripleHelixScheduler(&scheduler.Config{
		MaxConcurrentTasks: 100,
		PriorityDimensions: []string{"urgency", "importance", "efficiency", "energy", "trust"},
		PreemptionEnabled:  true,
		MaxRetries:         3,
...
func TestSyscallDispatcher(t *testing.T) {
	t.Skip("Skipping syscall dispatcher test - requires complex mocking")
}
...
func TestConcurrentOperations(t *testing.T) {
	...
	for i := 0; i < numTasks; i++ {
		go func(id int) {
			...
			err := tripleHelixScheduler.ScheduleTask(task)
			if err != nil {
