...
func (h *securityHandler) TmpSign(ctx context.Context, req *pb.TmpSignRequest) (*pb.TmpSignResponse, error) {
	...
	result, err := h.securityManager.TpmSignEnhanced(ctx, req.Data, req.KeyId)
	if err != nil {
...
func (h *securityHandler) VerifyManifest(ctx context.Context, req *pb.VerifyManifestRequest) (*pb.VerifyManifestResponse, error) {
	...
	// Verify manifest
	result, err := h.securityManager.VerifyManifest(ctx, req.Manifest, req.Signature, req.PublicKey)
	if err != nil {
		if ctx.Err() == context.DeadlineExceeded {
			err = syscall.NewTimeoutError(operation)
		}
	...
	return &pb.VerifyManifestResponse{
		...
		TrustLevel:  result.TrustLevel,    // Enhanced: Return trust level
