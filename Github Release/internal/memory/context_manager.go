...
func (cm *ContextManager) Read(ctx context.Context, namespace, key string, version int64) (*ContextData, error) {
	...
	// Validate namespace and key format (H-3 requirement)
	if err := cm.validateNamespaceAndKey(namespace, key); err != nil {
...
func (cm *ContextManager) ensureNamespace(namespace string) error {
	// Validate namespace before creation
	if err := cm.validateNamespace(namespace); err != nil {
		return err
	}
...
// validateNamespaceAndKey validates namespace and key format to prevent directory traversal
func (cm *ContextManager) validateNamespaceAndKey(namespace, key string) error {
	return validation.ValidateNamespaceAndKey(namespace, key)
}

// validateNamespace validates namespace format
func (cm *ContextManager) validateNamespace(namespace string) error {
